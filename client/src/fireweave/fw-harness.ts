 ...
 * the app's OWN build-baked `import.meta.env` signal (Vite `MODE`), so `staging`
 * builds bind the connected vendor as a first-class prod-tier surface — no
 * `PUBLIC_FW_ENV` var required (it stays an optional override only).
 *
...
const FW_SURFACES = [{ surfaceId: 'sfc_01KZVNRR9JG8RGJC9EQ4CZJWJ4', stamps: FW_STAMPS }];
...
// a FireWeave-specific `PUBLIC_FW_ENV` / `VITE_FW_ENV`. This app uses Vite's
// native `MODE` ('development' locally, 'production' when built), so
// `readEnvSignal` reads that directly. `PUBLIC_FW_ENV` / `VITE_FW_ENV` are
// retained ONLY as an optional override. Regenerated on `--reinit` — edit via
// the skill.
// fw:env-source
...
// Vite MODE=production maps to the FireWeave environment named 'prod'.
// MODE=development has no FW_ENV_PROFILES row by design — it falls through to
// the isProd() fallback below, which correctly classifies it as dev.
const FW_ENV_ALIASES: Record<string, string> = { production: 'prod' };
// ─────────────────────────────────────────────────────────────────────────────
...
export async function initFwHarness(): Promise<void> {
  ...
  // vendor. No observability vendor is bound yet for this project (INIT-B11
  // deferral, confirmed at init) — stay on console/no-op until one is bound.
  ...
  initFwAttestation({
    ...
    // Beacon provisioning was deferred at init (deploy detection is off) — this
    // resolves to a no-op until it's provisioned.
