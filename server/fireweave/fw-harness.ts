...
const FW_SURFACES = [{ surfaceId: 'sfc_01KZVNRR9JG8RGJC9EQ4CZJWJ3', stamps: FW_STAMPS }];
...
// introduce a FireWeave-specific `FW_ENV`. This repo already sets NODE_ENV via
// its npm scripts (`development` in dev, `production` in the built start
// script), so `readEnvSignal` reads that directly; `FW_ENV` is retained ONLY as
// an optional last-resort override. Regenerated on `--reinit` — edit via the
// skill.
// fw:env-source
...
// This repo's NODE_ENV=production maps to the FireWeave environment named
// 'prod'. NODE_ENV=development has no FW_ENV_PROFILES row by design — it falls
// through to the isProd() fallback below, which correctly classifies it as dev.
...
export async function initFwHarness(): Promise<void> {
  ...
  //    to the vendor. No observability vendor is bound yet for this project
  //    (INIT-B11 deferral, confirmed at init) — stay on console/no-op until one
  //    is bound and `--reinit` is run.
  ...
  //    prod-tier runtime env. Beacon provisioning was deferred at init (deploy
  //    detection is off) — this resolves to a no-op until it's provisioned.
  initFwAttestation({
    stamps: FW_STAMPS,
